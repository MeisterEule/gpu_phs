...
void do_verify_against_whizard (const char *ref_file, int n_x, int n_channels, int filepos_start_mom) {
   ...
   double *E1 = (double*)malloc(n_events * sizeof(double));
   double *E2 = (double*)malloc(n_events * sizeof(double));
   for (int i = 0; i < n_events; i++) {
      E1[i] = 500;
      E2[i] = 500;
      sqrts[i] = 2*sqrt(E1[i]*E2[i]);
   }
   ...
   double t1 = mysecond();
   gen_phs_from_x_gpu (false, E1, E2, n_events, n_channels, channels, n_x, x, factors, volumes, oks, p, x_out);
   double t2 = mysecond();
...
void do_verify_internal (size_t n_events_per_channel, int n_trials, size_t n_trial_events,
   ...
   for (int i = 0; i < n_trials; i++) {
      ...
      gen_phs_from_x_gpu (false, E1, E2, n_trial_events_tot, n_channels, channels,
...
// Reset GPU timers
   ...
   t1 = mysecond();
   gen_phs_from_x_gpu (false, E1, E2, n_events, n_channels, channels, n_x, x, factors_gpu, volumes_gpu, oks_gpu, p_gpu, x_out);
   t2 = mysecond();
   printf ("GPU: %lf sec\n", t2 - t1);
   printf ("   Memcpy In: %lf\n", gpu_timers[TIME_MEMCPY_IN]);
   printf ("   Memcpy Out: %lf\n", gpu_timers[TIME_MEMCPY_OUT]);
...
int main (int argc, char *argv[]) {
   ...
   for (int c = 0; c < n_channels; c++) {
      ...
      fprintf (logfl[LOG_INPUT], "\nfriends: ");
      for (int i = 0; i < N_PRT; i++) {
         fprintf (logfl[LOG_INPUT], "%d ", friends[c][i]);
      }
      fprintf (logfl[LOG_INPUT], "\nmappings: ");
